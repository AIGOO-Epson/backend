...
1. 학습자료 키워드를 3개 이상부터 doc.end가 호출해도, 콜백이 실행이 안됨. 뭐지? pdf.service.ts 에서 수정중

2. 원본 텍스트, 한국어 분석 텍스트 length맞추기도 추가하자. korean.service.ts에서 수정중

3. 배포시에 클라이언트로 보낸 인증쿠키가 클라이언트 페이지에서 새로고침하면 날라가버림. auth.service.ts 에서 쿠키설정 수정중
